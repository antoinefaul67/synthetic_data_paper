...
cat("Misclassification rate:", misclassification_rate_gmcm, "\n")
# 
...
# Function to run simulation with error handling
run_simulation <- function(n = 1000, m = 3, d = 2) {
  tryCatch({
    ...
    return(c(misclassification_rate, AMI_GMCM, ARI_GMCM, 
             misclassification_rate_vcmm, AMI_VCMM, ARI_VCMM, 
             misclassification_rate_gmm, AMI_GMM, ARI_GMM, 
             misclassification_rate_kmeans, AMI_Kmeans, ARI_Kmeans))
  }, error = function(e) {
...
while (length(results) < n_repetitions) {
...
# Convert list of results to matrix for easier manipulation
misclassification_results <- do.call(rbind, results)

mean_misclassification_rate_GMCM <- mean(misclassification_results[,1])
mean_AMI_GMCM <- mean(misclassification_results[,2])
mean_ARI_GMCM <- mean(misclassification_results[,3])
mean_misclassification_rate_VCMM <- mean(misclassification_results[,4])
mean_AMI_VCMM <- mean(misclassification_results[,5])
mean_ARI_VCMM <- mean(misclassification_results[,6])
mean_misclassification_rate_GMM <- mean(misclassification_results[,7])
mean_AMI_GMM <- mean(misclassification_results[,8])
mean_ARI_GMM <- mean(misclassification_results[,9])
mean_misclassification_rate_K_means <- mean(misclassification_results[,10])
mean_AMI_K_means <- mean(misclassification_results[,11])
mean_ARI_K_means <- mean(misclassification_results[,12])
